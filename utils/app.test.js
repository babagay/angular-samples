 ...
 *
 * [зачем нужны шпионы]
 * 1) Мы сконструировали мок-объект и дергаем его через интерфейсные методы, но
 * хотим проверить, используются ли в процессе вызова какие-то внутренние методы (приватные)
 * 2) Если у нас есть генератор событий и обработчик.
 * Мы можем повесить спая на этот хандлер и сгенерировать событие.
 * И тогла, проверить, вызывается ли обработчик.
 */
